...
async background(widget) {
  ...
  if (current < code.data.sun.sunrise) {
    const path = files.joinPath(dirPath, "Weather Cal-dark.jpg")
  return widget.backgroundImage = files.readImage(path)
  }
  
  if (current < code.data.sun.sunset) {
    const path = files.joinPath(dirPath, "Weather Cal-light.jpg")
  return widget.backgroundImage = files.readImage(path)
  }
